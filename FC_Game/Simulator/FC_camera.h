...
public:
	...
	inline CvPoint dpt_real(CvPoint& xy){
		return CvPoint((d*c + h)*xy.x / (s*xy.y + d * c),
			((d*c + h)*xy.y - h * d*s) / (s*xy.y + d * c));
	}

	inline CvPoint pt_real(CvPoint& xy) {
		return CvPoint(d*xy.x / (d + h * c - s * xy.y),
			d*(c*xy.y + h * s) / (d + h * c - s * xy.y));
	}

	inline CvPoint dpt_point(IplImage*perspective, const IplImage*overlook, CvPoint &xy) {
	...
	inline CvPoint pt_point(const IplImage*perspective, IplImage*overlook, CvPoint& xy) {
...
private:
	...
	CvPoint xytoij(const IplImage*img, CvPoint &xy) {
		return CvPoint(xy.x + img->width / 2, img->height - xy.y);
	}

	CvPoint ijtoxy(const IplImage*img, CvPoint &ij) {
