...
class DDPG(object):
    def __init__(self, s_dim, a_dim, a_bound):
        self.REPLAY_CAPACITY=12800
        self.replay = np.zeros((self.REPLAY_CAPACITY, s_dim * 2 + a_dim + 1), dtype=np.float32)
        self.frame = 0
    ...
    def actor_net(self, s, reuse=None, custom_getter=None):
        ...
        with tf.name_scope('Actor'):
            with tf.variable_scope('vActor', reuse=reuse, custom_getter=custom_getter):
                a = tf.layers.dense(s, 64, activation=tf.nn.tanh, trainable=trainable, kernel_regularizer=tf.nn.l2_loss )
                a = tf.layers.dense(a, self.a_dim, activation=tf.nn.sigmoid, trainable=trainable, kernel_regularizer=tf.nn.l2_loss)
    ...
    def critic_net(self, s, a, reuse=None, custom_getter=None):
        ...
        with tf.name_scope('Critic'):
            with tf.variable_scope('vCritic', reuse=reuse, custom_getter=custom_getter):
                net=tf.concat([s,a], 1)
                net = tf.layers.dense(net, 64, activation=tf.nn.tanh, trainable=trainable, kernel_regularizer=tf.nn.l2_loss)
                return tf.layers.dense(net, 1, trainable=trainable, kernel_regularizer=tf.nn.l2_loss)
                
    def predict(self, s):
        return self.sess.run([self.a, self.q], feed_dict={self.S:s[np.newaxis, :]})
